            ...
            subcommandGroup =>
                subcommandGroup.setName('tag').setDescription('Tag group')
                    .addSubcommand(subcommand =>
                        subcommand.setName('add').setDescription('如何新增標籤')
                    )
                    .addSubcommand(subcommand =>
                        subcommand.setName('remove').setDescription('如何移除標籤')
                    )
                    .addSubcommand(subcommand =>
                        subcommand.setName('show').setDescription('如何列出標籤')
                    )
                    .addSubcommand(subcommand =>
                        subcommand.setName('info').setDescription('如何列出標籤資訊')
                    )
                    .addSubcommand(subcommand =>
                        subcommand.setName('fetch').setDescription('如何取得標籤對應問題及結果')
                    )
                    .addSubcommand(subcommand =>
                        subcommand.setName('all').setDescription('tag各功能簡易介紹')
                    )
                    .addSubcommand(subcommand =>
                        subcommand.setName('edit').setDescription('如何編輯標籤')
                    )
        )
        .addSubcommand(subcommand =>
            subcommand.setName('info').setDescription('如何取得網站更多資訊')
        )
        .addSubcommand(subcommand =>
            subcommand.setName('ask').setDescription('如何提問問題')
        )
        .addSubcommand(subcommand =>
            subcommand.setName('all').setDescription('如何取得歷史資訊')
        ),
